...
if ( !function_exists( 'wbc_extended_example' ) ) {
	function wbc_extended_example( $demo_active_import , $demo_directory_path ) {
		...
		*************************************************************************/
		...
		//         'demo1' => array('fullwidth.zip','homeslider_boxwidth.zip'),
		//     );
		...
		*************************************************************************/
		...
		if ( isset( $demo_active_import[$current_key]['directory'] ) && !empty( $demo_active_import[$current_key]['directory'] ) && in_array( $demo_active_import[$current_key]['directory'], $wbc_menu_array ) ) {
			...
			if ( isset( $primary_menu->term_id ) ) {
				$menu_array['primary'] = $primary_menu->term_id;
			}
			// secondary_nav
			$left_menu = get_term_by( 'name', 'Secondary Menu', 'nav_menu' );
			if ( isset( $left_menu->term_id ) ) {
				$menu_array['secondary_nav'] = $left_menu->term_id;
			}
